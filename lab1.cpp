...
void makeParticle(int x, int y)
{
	...
	p->s.center.x = x;
	p->s.center.y = y;
	p->velocity.y = (float) rand() / RAND_MAX - 0.5f ;
	p->velocity.x = (float) rand() / RAND_MAX - 0.5f ;
	++g.n;
}
...
void movement()
{
	...
	for ( int i = 0 ; i < g.n ; i ++ ) {
		...
		//Shape *s;
		Shape *s = &g.box;
		bool hasCollision =  p->s.center.y < ( s->center.y  + s->height ) ? 
	        	                                        p->s.center.y > ( s->center.y  - s->height ) ?
				                                   p->s.center.x > ( s->center.x  - s->width  ) ?
			    	                                   p->s.center.x < ( s->center.x  + s->width  ) 
			    	                                   : 0 : 0 : 0 ; 
		if ( hasCollision ) p->velocity.y = ( -p->velocity.y ) * 0.5 ; 
